...
func TestAccAllResourcesAndDataSources(t *testing.T) {
	...
	config := func(suffix string, replicas int, optionals bool) string {
		...
		optionalApplication := ""
		if optionals {
			optionalAccount = `description = "updated account"`
			optionalApplication = `
...
resource "vappcloud_device" "test" {
  account_id = vappcloud_account.test.id
  name       = "device-%s"
}
...
data "vappcloud_accounts" "all" {
  depends_on = [vappcloud_account.test]
}
data "vappcloud_account" "selected" { id = vappcloud_account.test.id }
...
data "vappcloud_cloud_connections" "all" { account_id = vappcloud_account.test.id }
...
data "vappcloud_github_connections" "all" { account_id = vappcloud_account.test.id }
data "vappcloud_github_repositories" "all" { github_connection_id = "github-test" }
`, acceptanceProviderBlock(server.URL), suffix, optionalAccount, suffix, suffix, suffix, suffix, optionalApplication, replicas)
	}
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("vappcloud_account.test", "resource_version", "2"),
